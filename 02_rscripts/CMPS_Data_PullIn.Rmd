...
hardship_values <- c("Often", "Sometimes", "Occasionally")

# Recode the frequency variables to binary
cmps2016[state_vars] <- lapply(cmps2016[state_vars], function(x) {
  x <- tolower(as.character(x))  # ensure uniform text
  ifelse(x %in% hardship_values, 1,
         ifelse(x == "Never", 0, NA))  # leave other responses as NA
})

# Combine all hardship-related variables
all_hardship_vars <- c(predatory_vars, state_vars)
...
hardship_data_2016 <- cmps2016[, all_hardship_vars]
...
hardship_summary_2016 <- data.frame(
  Hardship_Variable = all_hardship_vars,
